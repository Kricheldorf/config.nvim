...
-- TODO:ini
-- setup multicursors
-- setup exchange - check vim-exchange
-- check options for surround plugin - search this file for mini-surround
...
require('lazy').setup({
  ...
  { -- Fuzzy finder + UI suite (replaces telescope)
    'folke/snacks.nvim',
    priority = 1000,
    lazy = false,
    ---@type snacks.Config
...
-- Print keybinds to file
-- local modes = {'n','i','v','x','s','o','c','t'}
-- local lines = {}
-- for _, m in ipairs(modes) do
--   table.insert(lines, '')
--   table.insert(lines, '## Mode: ' .. m)
--   table.insert(lines, '')
--   table.insert(lines, '| LHS | RHS / Desc | Source |')
--   table.insert(lines, '|-----|------------|--------|')
--   for _, map in ipairs(vim.api.nvim_get_keymap(m)) do
--     local rhs = map.rhs or map.desc or '<lua function>'
--     local src = map.desc or ''
--     table.insert(lines, string.format('| `%s` | %s | %s |',
--       map.lhs, (rhs):gsub('|','\\|'), src))
--   end
-- end
-- vim.fn.writefile(lines, vim.fn.expand('~/nvim-keymaps.md'))
-- print('Wrote ~/nvim-keymaps.md')
